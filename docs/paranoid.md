...
Execute the following commands. Note that you might want to change the volume `$PWD` or use a data docker container for this.
    ...
    docker run --net=none --rm -t -i -v $PWD:/etc/openvpn arubislander/armhf-openvpn ovpn_genconfig -u udp://VPN.SERVERNAME.COM
    docker run --net=none --rm -t -i -v $PWD:/etc/openvpn arubislander/armhf-openvpn ovpn_initpki
    docker run --net=none --rm -t -i -v $PWD:/etc/openvpn arubislander/armhf-openvpn ovpn_copy_server_files
...
The following options have been tested successfully:
    ...
    docker run --volumes-from $OVPN_DATA --net=none --rm arubislander/armhf-openvpn ovpn_genconfig -C 'AES-256-CBC' -a 'SHA384'
